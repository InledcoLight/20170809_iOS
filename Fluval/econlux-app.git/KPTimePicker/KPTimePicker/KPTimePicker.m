...
#define kDefLightColor          CELL_BACKGROUND_COLOR
#define kDefDarkColor           DARK_COLOR
#define kDefConfirmColor        [UIColor colorWithRed:0.434 green:0.563 blue:1.000 alpha:1.000]
#define kDefButColor			[UIColor colorWithRed:0.117 green:0.254 blue:0.597 alpha:1.000]
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		...
		[self.confirmButton setBackgroundImage:[kDefButColor image] forState:UIControlStateNormal];
...
//        [self.confirmButton setImage:[UIImage imageNamed:@"checkmark_icon_white"] forState:UIControlStateHighlighted];
		[self.confirmButton setTitle:APLocString(@"TIME_BUTTON_SET") forState:UIControlStateNormal];
		...
		self.labelTop = [[UILabel alloc] initWithFrame:CGRectMake(0, 20, self.bounds.size.width, 120)];
		[self.labelTop setTextAlignment:NSTextAlignmentCenter];
		[self.labelTop setTextColor:[UIColor whiteColor]];
		[self.labelTop setFont:[UIFont fontWithName:@"Roboto-Light" size:15]];
		[self.labelTop setNumberOfLines:5];
		[self.labelTop setMinimumScaleFactor:.5f];
...
-(void)layoutSubviews{
    ...
    self.clockLabel.frame = CGRectMake(0, spacing+heightForDay + PADDING_TOP, self.bounds.size.width, heightForTime);
	self.labelTop.frame = CGRectMake(0, 10, self.bounds.size.width, self.clockLabel.y + PADDING_TOP * 0.8);
